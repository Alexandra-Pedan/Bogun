...
function createBuildCard(build) {
  return `<a class="building-progress-item-wrap js-build-card" href="#" data-build-id="${build.id}">
    <div class="building-progress-item">
      <div class="building-progress-img img-active" style="background: linear-gradient(180deg, rgba(59, 65, 78, 0.9) 0%, rgba(59, 65, 78, 0) 100%), url(${build.slider[0]})"></div>
...
function loadMoreHandler(state, containers) {
  const count = (state.countShowBuild + 10 < state.builds.length) ? 10 : state.builds.length - state.countShowBuild;
...
async function initBuild() {
  ...
  containers.buildContainer.addEventListener('click', event => buildContainerHandler(event, state, containers), {});
